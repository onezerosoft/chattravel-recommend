...
dff = dfs.dropna(subset = ['LODGING_TYPE_CD'])  # 숙박 유형이 없는 변수 제거
...
# Filter out rows containing any of the specified words
df2 = fdf[~fdf['VISIT_AREA_NM'].str.contains('|'.join(words_to_remove3))]

df2['SIDO'] = df2['LOTNO_ADDR'].str.split().str[0]
df2['SI'] = df2['LOTNO_ADDR'].str.split().str[1]
  

# 전처리에 필요한 데이터만 가져와 특정 변수에 저장
df2 = df2[['TRAVEL_ID', 'VISIT_AREA_NM','DGSTFN','SIDO','SI']]

df2.columns = ['userID', 'itemID', 'rating','SIDO','SI']
...
# df2.to_csv('data/preprocessed/숙박/dfE.csv')
# df2.to_csv('data/preprocessed/숙박/dfF.csv')
# df2.to_csv('data/preprocessed/숙박/dfG.csv')
df2.to_csv('data/preprocessed/숙박/dfH.csv')